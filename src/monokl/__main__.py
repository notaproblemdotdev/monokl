"""Entry point for Monokl.

Run the dashboard with: python -m monokl dash
Run setup with: python -m monokl setup
"""
...
from monokl import configure_logging
...
from monokl.features import get_feature_flag
from monokl.features import is_feature_enabled
from monokl.logging_config import get_log_file_path
from monokl.tools import tools_app
from monokl.ui.app import MonoApp


class MonoklApp(typer.Typer):
    ...
    def command(
        ...
        *,
        ...
        def decorator(func: t.Callable[..., t.Any]) -> t.Callable[..., t.Any]:
            ...
            return super(MonoklApp, self).command(
                name=name,
                cls=cls,
...
def _apply_env_vars(offline: bool, db_path: str | None) -> None:
    if offline:
        os.environ["MONOKL_OFFLINE_MODE"] = "true"
    if db_path:
        os.environ["MONOKL_DB_PATH"] = db_path
...
def dash(
    ...
    if web:
        ...
        cmd_parts = [sys.executable, "-m", "monokl", "dash"]
        if debug:
...
def run_dev() -> None:
    """Run monokl with Textual hot reload (for development)."""
    env = os.environ.copy()
    env["PYTHONPATH"] = "src"

    result = subprocess.run(
        ["uv", "run", "textual", "run", "--dev", "monokl.ui.dev:app"],
        check=False,
        env=env,
    )
