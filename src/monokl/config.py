"""Configuration management for monokl.
...
from monokl.config_models import AppConfig

logger = get_logger(__name__)

# Default config locations (in priority order)
CONFIG_PATHS = [
    Path.home() / ".config" / "monokl" / "config.yaml",
    Path.home() / ".monokl.yaml",
    Path.cwd() / ".monokl.yaml",
]
...
class Config:
    """Configuration manager for monokl.

    Loads configuration from environment variables and YAML config files.
    Environment variables take precedence over config file values.

    Environment variables:
        MONOKL_GITLAB_GROUP: Default GitLab group to fetch MRs from
        MONOKL_GITLAB_PROJECT: Alias for GitLab group/scope
        MONOKL_JIRA_PROJECT: Default Jira project to fetch issues from
        MONOKL_TODOIST_TOKEN: Todoist API token
    ...
    def _apply_env_vars(cls, data: dict[str, Any]) -> dict[str, Any]:
        ...
        # GitLab settings
        gitlab_group = os.getenv("MONOKL_GITLAB_GROUP")
        if gitlab_group:
            data["gitlab"]["group"] = gitlab_group
        gitlab_project = os.getenv("MONOKL_GITLAB_PROJECT")
        if gitlab_project and "group" not in data["gitlab"]:
            data["gitlab"]["group"] = gitlab_project

        # Jira settings
        jira_project = os.getenv("MONOKL_JIRA_PROJECT")
        if jira_project:
            data["jira"]["project"] = jira_project
        jira_base_url = os.getenv("MONOKL_JIRA_BASE_URL")
        if jira_base_url:
            data["jira"]["base_url"] = jira_base_url

        # Todoist settings
        todoist_token = os.getenv("MONOKL_TODOIST_TOKEN")
        if todoist_token:
        ...
        feature_experimental = os.getenv("MONOKL_FEATURE_EXPERIMENTAL")
    ...
    def todoist_token(self) -> str | None:
        ...
        1. Environment variable MONOKL_TODOIST_TOKEN (highest priority)
        ...
        """
        env_token = os.getenv("MONOKL_TODOIST_TOKEN")
        if env_token:
            return env_token
    ...
    def azuredevops_token(self) -> str | None:
        ...
        """
        env_token = os.getenv("MONOKL_AZUREDEVOPS_TOKEN")
        if env_token:
            return env_token
    ...
    def cache_ttl(self) -> int:
        ...
        env_ttl = os.getenv("MONOKL_CACHE_TTL")
        if env_ttl:
            try:
                return int(env_ttl)
            except ValueError:
                logger.warning("Invalid MONOKL_CACHE_TTL value", value=env_ttl)
        return self._model.cache.ttl_seconds

    @property
    def offline_mode(self) -> bool:
        """Get whether offline mode is enabled."""
        return os.getenv("MONOKL_OFFLINE_MODE", "").lower() in ("true", "1", "yes")

    @property
    def experimental_features(self) -> bool:
        """Get whether experimental features are enabled."""
        env_val = os.getenv("MONOKL_FEATURE_EXPERIMENTAL", "").lower()
    ...
    def require_gitlab_group(self) -> str:
        ...
        if not group:
            raise ConfigError(
                ...
                "\nSet one of:\n"
                "  - Environment variable: export MONOKL_GITLAB_GROUP='your-group'\n"
                "  - Config file: ~/.config/monokl/config.yaml\n"
                "\nConfig file format:\n"
                "  gitlab:\n"
                "    group: your-group"
            )
    ...
    def require_jira_base_url(self) -> str:
        ...
        if not base_url:
            raise ConfigError(
                ...
                "\nSet one of:\n"
                "  - Environment variable: export MONOKL_JIRA_BASE_URL='https://your-company.atlassian.net'\n"
                "  - Config file: ~/.config/monokl/config.yaml\n"
                "\nConfig file format:\n"
                "  jira:\n"
...
def validate_keyring_available() -> None:
    ...
    if os.getenv("MONOKL_TODOIST_TOKEN"):
        return
    ...
    if not keyring_utils.is_available():
        raise ConfigError(
            ...
            "monokl requires secure credential storage for Todoist. Please ensure:\n"
            ...
            "  - Set MONOKL_TODOIST_TOKEN environment variable\n"
